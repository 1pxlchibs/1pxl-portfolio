...
const createProject = () : string[] => {
  ...
  command.client_projects.forEach((ele) => {
    let link = `<a href="${ele[2]}" target="_blank">${ele[0]}</a>`
    string += SPACE.repeat(2);
    string += link;
    string += SPACE.repeat(45 - ele[0].length);
  ...
  projects.push("<br>");
  projects.push(files);
  projects.push("To learn more about a project type <span class='command'>'more &lt;project name&gt;'</span>");
  projects.push("<br>");
  return projects
}
